...
const DEFAULT_OUTPUT_JSON = ".runtime-cache/artifacts/ci/precommit-strict-weekly-summary.json";
const DEFAULT_OUTPUT_MD = ".runtime-cache/artifacts/ci/precommit-strict-weekly-summary.md";
...
function summarize(records, options) {
  ...
  const filtered = records.filter((r) => {
    if (options.mode !== "all" && r.mode !== options.mode) return false;
  ...
  const durationMsList = filtered
    ...
    .sort((a, b) => a - b);
  const avgDurationMs = durationMsList.length === 0
    ? 0
    : Math.round(durationMsList.reduce((a, b) => a + b, 0) / durationMsList.length);

  const lastRun = filtered
    .slice()
    .sort((a, b) => (asTime(b.timestamp) || 0) - (asTime(a.timestamp) || 0))[0] || null;
...
function toMarkdown(summary) {
  ...
  const lines = [];
  lines.push("# Pre-commit Strict Weekly Summary");
  lines.push("");
